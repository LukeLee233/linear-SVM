...
flags.DEFINE_float('svmC', 1,
                   'The C parameter of the SVM cost function.')

flags.DEFINE_boolean('verbose', False, 'Produce verbose output.')
flags.DEFINE_boolean('plot', True, 'Plot the final decision boundary on the data.')
...
def main(argv=None):
    ...
    # the classification.
    with tf.variable_scope('trainable-variable'):
        W = tf.get_variable('W', shape=[num_features, 1],
    ...
    # before launch the graph
    merged = tf.summary.merge_all()
    init = tf.global_variables_initializer()

    # Create a local session to run this computation.
    with tf.Session() as s:
        with tf.summary.FileWriter('./logs',graph=s.graph) as writer:
            ...
            for step in range(num_epochs * train_size // BATCH_SIZE):
                ...
                _, statics = s.run([train_step, merged], feed_dict={x: batch_data, y: batch_labels})
