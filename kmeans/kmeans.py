...
X_mat = np.random.gamma(shape=2, size=[n_obs, 2])
plt.scatter(X_mat[:, 0], X_mat[:, 1], alpha=0.6)
with tf.variable_scope('Inputs'):
    X = tf.placeholder(tf.float32, [X_mat.shape[0], X_mat.shape[1]], name='observations')
    centroids = tf.Variable(tf.random_uniform([k, 2]), dtype=tf.float32, name='centroids')

# reshaping data to get distances to centroids
with tf.variable_scope('reshaping-layer'):
    X_temp = tf.reshape(tf.tile(X, [1, k]), [n_obs, k, 2])
    centroids_temp = tf.reshape(tf.tile(centroids, [n_obs, 1]), [n_obs, k, 2])

# calculate distances(element-wise), find nearest centroid for each point and assign membership
with tf.variable_scope("distance-layer"):
    distances_to_centroids = tf.reduce_sum(tf.square(
        tf.subtract(X_temp, centroids_temp)), axis=-1)  # [N x k]
    cluster_membership = tf.arg_min(distances_to_centroids, 1,
                                    name='new-cluster')  # [N,] distance-minimizing column for each row

# update centroids by moving them to the mean of their now-updated points
with tf.name_scope("New-centroids"):
    ...
    delta_op = tf.reduce_sum(tf.abs(tf.subtract(centroids, new_means)))
    tf.summary.scalar('delta', delta_op)
...
with tf.Session() as sess:
    with tf.summary.FileWriter('./logs', graph=sess.graph) as writer:
        sess.run(init)
        delta = sess.run([delta_op], feed_dict={X: X_mat})[0]
        step = 0
        while delta > 1e-5:
            print(delta)
            statics = sess.run(merged, feed_dict={X: X_mat})

            writer.add_summary(statics, step)
            step += 1

            sess.run([update_centroids], feed_dict={X: X_mat})
            print(centroids.eval())
            delta = sess.run([delta_op], feed_dict={X: X_mat})[0]

        # print centroids
        print("final centroids: \n" + str(centroids.eval()))
        plt.scatter(centroids.eval()[:, 0], centroids.eval()[:, 1], alpha=0.6, color="red")
        plt.show()
